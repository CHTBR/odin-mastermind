  ...
  describe "#input" do
    ...
    context "during a normal game" do
      ...
      xit "returns [:red, :green, :red, :red] after getting evaluations 3, 2, 4]" do
        allow(@guess_evaluator_double).to receive(:input).and_return({ color_and_spot: 3 }, { color_and_spot: 2 },
                                                                     { color_and_spot: 4 })
        3.times { @computer.input }
      ...
      xit "returns [:yellow, :green, :red, :red] after getting evaluations 2, 2, 2, 4]" do
        allow(@guess_evaluator_double).to receive(:input).and_return({ color_and_spot: 2 }, { color_and_spot: 2 },
                                                                     { color_and_spot: 2 }, { color_and_spot: 4 })
        4.times { @computer.input }
      ...
      xit "returns [:red, :red, :red, :green] after getting evaluations 3, 2, 2, 2, 4]" do
        allow(@guess_evaluator_double).to receive(:input).and_return({ color_and_spot: 3 }, { color_and_spot: 3 },
                                                                     { color_and_spot: 3 }, { color_and_spot: 3 },
                                                                     { color_and_spot: 4 })
      ...
      xit "returns [:yellow, :red, :red, :red] after getting evaluations 3, 3, 3, 4]" do
        allow(@guess_evaluator_double).to receive(:input).and_return({ color_and_spot: 3 }, { color_and_spot: 3 },
                                                                     { color_and_spot: 3 }, { color_and_spot: 4 })
        4.times { @computer.input }
